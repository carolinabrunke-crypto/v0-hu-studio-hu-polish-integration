...
import tomasEspinosaPhoto from "../fotos-perfil/Tomas Espinosa.png";
import tomasMeszelPhoto from "../fotos-perfil/Tomas Meszel.png";
...
export default function Presentation() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute left-0 top-1/3 w-64 h-64 pointer-events-none opacity-20 transition-all duration-300 ${videoLightboxOpen ? "blur-sm opacity-10" : ""}`}
      >
      ...
      {/* Header */}
      <header
        className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-12 flex items-center justify-between backdrop-blur-sm transition-all duration-300 ${videoLightboxOpen ? "blur-sm opacity-50" : ""}`}
        style={{
      ...
      >
        ...
        {currentSlide > 0 && (
          ...
          >
            {navSteps.map((step, index) => {
              ...
              const isPast =
                currentPhase && parseInt(currentPhase) > parseInt(step.num);
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm" style={{ color: "#3A3A45" }}>
              Activo
            </span>
          </div>
      ...
      {/* Slide Content */}
      <div
        className={`flex-1 flex items-center justify-center px-6 md:px-12 lg:px-24 pt-24 pb-20 duration-300 ${videoLightboxOpen ? "brightness-100" : ""}`}
      >
        <div
          key={currentSlide}
          style={{ animation: "fadeInBlur 0.55s ease forwards" }}
        >
          {currentSlide === 0 && <CoverSlide onNext={nextSlide} />}
          {currentSlide === 1 && <ProblemSlide />}
          {currentSlide === 2 && <SolutionSlide />}
          {currentSlide === 3 && (
            <FlowSlide onVideoOpen={setVideoLightboxOpen} />
          )}
      ...
      <footer
        className={`fixed bottom-0 left-0 right-0 px-6 md:px-12 py-4 flex items-center justify-center backdrop-blur-sm transition-all duration-300 ${videoLightboxOpen ? "blur-sm opacity-50" : ""}`}
        style={{
          background: "rgba(247, 247, 247, 0.85)",
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-500 hover:scale-110 ${
                currentSlide === index ? "w-6" : "w-2"
              }`}
              style={{
                backgroundColor: currentSlide === index ? "#182D7A" : "#D6E1FF",
                animation:
                  currentSlide === index ? "scaleIn 0.3s ease" : "none",
              }}
            />
          ))}
        </div>
...
function PhaseBadge({ phase, title }: { phase: string; title: string }) {
  return (
    <div
      className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-6 relative overflow-hidden"
      style={{
    ...
    >
      <Sparkles
        className="w-4 h-4"
        style={{ animation: "sparkleScale 2s ease-in-out infinite" }}
      />
      <span>
        FASE {phase} — {title}
      </span>
...
function CoverSlide({ onNext }: { onNext: () => void }) {
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      >
        Ecosistema autónomo de producción audiovisual con IA. Automatiza
        pre-producción, control de calidad y localización.
      </p>
      <button
        onClick={onNext}
...
function ProblemSlide() {
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      <h2
        className="text-4xl md:text-6xl lg:text-7xl font-light tracking-tight mb-4 text-balance"
        style={{
          color: "#182D7A",
      ...
      >
        Los equipos multimedia pierden semanas en tareas repetitivas que la IA
        puede automatizar.
      </p>
      ...
      <div className="grid md:grid-cols-3 gap-6 text-left">
        {[
          {
            num: "01",
            title: "Pre-producción lenta",
            desc: "Guiones, storyboards y locuciones requieren semanas de trabajo manual.",
            delay: "0.1s",
          },
          {
            num: "02",
            title: "Errores costosos",
            desc: "Errores en motion design detectados post-render causan re-trabajos.",
            delay: "0.2s",
          },
          {
            num: "03",
            title: "Escalar es multiplicar",
            desc: "Adaptar a múltiples idiomas significa repetir todo el proceso.",
            delay: "0.3s",
          },
        ].map((item, index) => (
          <div
          ...
          >
            <div
              className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium mb-4 relative"
              style={{
                backgroundColor: "#D6E1FF",
                color: "#182D7A",
              }}
            ...
            <h3 className="font-medium mb-2" style={{ color: "#182D7A" }}>
              {item.title}
            </h3>
...
function SolutionSlide() {
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      >
        hustudio combina generación de contenido con IA y control de calidad
        para motion design.
      </p>
      ...
      <div className="grid md:grid-cols-2 gap-6 text-left">
        ...
        >
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#3A3A45" }}>
            De un PDF a guion, storyboard, imágenes y locución alineados con tu
            brandbook. Automáticamente.
          </p>
          <div
        ...
        >
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#3A3A45" }}>
            Plugin de After Effects que detecta errores antes del render.
        ...
        >
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#3A3A45" }}>
            Doblaje de voz y subtitulado en múltiples idiomas. Escala la
            comunicación de Humand a nivel global.
          </p>
          <div
            className="absolute inset-0 w-[200%] h-full opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
            style={{
...
function FlowSlide({ onVideoOpen }: { onVideoOpen: (open: boolean) => void }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      video: "/videos/Guión.mp4",
    },
    {
      num: "03",
      title: "Storyboard",
      desc: "Imágenes + brandbook",
      video: "/videos/Storyboard.mp4",
    },
    {
      num: "04",
      title: "Clips",
      desc: "Redes, ads y outreach",
      video: "/videos/Clips.mp4",
    },
    {
      num: "05",
      title: "QA",
      desc: "Errores + feedback centralizado",
      videos: ["/videos/QA-2.mp4", "/videos/QA-3.mp4"],
    },
    {
      num: "06",
      title: "Global",
      desc: "Doblaje + subtítulos",
      videos: ["/videos/Global.mp4", "/videos/Global-2.mp4"],
    },
  ];
  ...
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      >
        Seis pasos automatizados para reducir al máximo la tarea manual del
        equipo multimedia.
      </p>
        ...
        {steps.map((step, index) => (
          <div key={step.num} className="flex items-center">
            ...
            >
              ...
              <h3
                className="font-medium mb-1 transition-colors hover:text-[#496BE3]"
                style={{ color: "#182D7A" }}
              >
                {step.title}
              </h3>
      ...
      <div
        className="mt-12 p-6 rounded-2xl inline-block border relative overflow-hidden group"
        style={{
      ...
      >
        <p className="text-sm relative z-10" style={{ color: "#182D7A" }}>
          <span className="font-medium">Objetivo:</span> El diseñador se enfoca
          solo en crear motion graphics de alta calidad.
        </p>
        <div
          className="absolute inset-0 w-[200%] h-full opacity-0 group-hover:opacity-100 transition-opacity"
          style={{
            background:
              "linear-gradient(90deg, transparent 0%, rgba(73,107,227,0.1) 50%, transparent 100%)",
            animation: "shimmerSweep 2s ease-in-out infinite",
          }}
        />
      </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                className={`relative rounded-2xl overflow-hidden w-full h-full ${selectedStep === "05" || selectedStep === "06" ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "flex items-center justify-center"}`}
                style={{
                  boxShadow:
                    "0 0 100px rgba(73,107,227,0.9), 0 0 200px rgba(73,107,227,0.6), 0 0 300px rgba(24,45,122,0.4)",
                ...
                {selectedStep === "05" ? (
                  ...
                  <>
                    ...
                    <video
                      ...
                      style={{ backgroundColor: "#000000" }}
                    >
                      <source src="/videos/QA-3.mp4" type="video/mp4" />
                    </video>
                  </>
                ) : selectedStep === "06" ? (
                  <>
                    ...
                    <video
                      ...
                      style={{ backgroundColor: "#000000" }}
                    >
                      <source src="/videos/Global-2.mp4" type="video/mp4" />
                    </video>
                  </>
                ) : (
                  ...
                  >
                    <source
                      src={steps.find((s) => s.num === selectedStep)?.video}
                      type="video/mp4"
                    />
                  </video>
                )}
...
function TeamSlide() {
  const team = [
    ...
    { name: "Nello De Piero Teti", photo: nelloDePieroTetiPhoto },
    { name: "Milena Yuri", photo: mileYuriPhoto },
    { name: "Tomas Espinosa", photo: tomasEspinosaPhoto },
    { name: "Tomas Meszel", photo: tomasMeszelPhoto },
  ];
  ...
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      <h2
        className="text-4xl md:text-6xl lg:text-7xl font-light tracking-tight mb-12 text-balance"
        style={{
          color: "#182D7A",
          animation: "fadeInUp 0.5s ease",
        }}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {team.map((member, index) => (
          ...
          >
            <div
              className="w-16 h-16 rounded-full mx-auto mb-3 transition-all duration-300 group-hover:scale-110 relative overflow-hidden"
              style={{
            ...
            <p className="text-sm font-medium" style={{ color: "#182D7A" }}>
              {member.name}
            </p>
            <div
              className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#496BE3] to-[#182D7A] opacity-0 group-hover:opacity-100 transition-all duration-300"
